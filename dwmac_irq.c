...
{
	DBG_UWB_IRQ("*** RX 0x%" PRIx32 " flags 0x%x", status->status,
				status->rx_flags);
...
{
	DBG_UWB_IRQ("*** RX TO 0x%" PRIx32, dat->status);
...
{
	DBG_UWB_IRQ("*** ERR 0x%x 0x%" PRIx32, dat->rx_flags, dat->status);
...
{
	DBG_UWB_IRQ("*** TX Done 0x%" PRIx32, dat->status);
	tx_done_cnt++;
